...
# 训练函数
# =========================
def train(model, device, batch_size=64, num_workers=4, epochs=10):
    all_losses = []
    ...
    if isinstance(model, nn.DataParallel):
        torch.save(model.module.state_dict(), PATH)  # 保存原始模型参数
    else:
...
def test(model, device, batch_size=64, num_workers=4):
    ...
    # 加载模型参数时处理 DataParallel 前缀
    state_dict = torch.load('./cifar10_mymodel.pth', map_location=device)
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        model.to(device)
        train(model, device, batch_size=args.batch_size, epochs=args.epochs)
    elif args.mode == "test":
        model.to(device)
        test(model, device, batch_size=args.batch_size)
